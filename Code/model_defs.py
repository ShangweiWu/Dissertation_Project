...
class GATv2Net(torch.nn.Module):
    def __init__(self, node_feature_dim, edge_feature_dim, config):
        ...
        # Change: Change the model to two output nodes
        self.reg_head = nn.Linear(256, 1)  # regression node
        self.cls_head = nn.Linear(256, 1)   # classification node 
    ...
    def forward(self, data):
        ...
        # Change: Change it in order to get two output for each task 
        pk_pred = self.reg_head(x)          # regression output
        pk_logits = self.cls_head(x)        # classification output
        
        return pk_pred, pk_logits           # return both outputs
