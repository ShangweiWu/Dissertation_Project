...
def predict(model, device, loader, y_scaler=None):
    ...
    # Change: Change to two outputs
    total_pKd_preds = torch.Tensor()      
    total_bind_logits = torch.Tensor()    

#   Change: Change total_graph_ids
    total_graph_ids = torch.empty(0, dtype=torch.long)
...
#    total_graph_ids = torch.IntTensor()
    ...
    with torch.no_grad():
        for data in loader:
            data = data.to(device)
            pk_pred, pk_logits = model(data)  # Change: Change to two nodes

            # Change: Change to two nodes

            total_pKd_preds = torch.cat((total_pKd_preds, pk_pred.cpu()), 0)
            total_bind_logits = torch.cat((total_bind_logits, pk_logits.cpu()), 0)
        
#            total_graph_ids = torch.cat((total_graph_ids, data.y.view(-1, 1).cpu()), 0)
#            total_graph_ids = torch.cat((total_graph_ids, data.y.view(-1).cpu()), 0)
#            Change: Change total_graph_ids
            total_graph_ids = torch.cat((total_graph_ids, data.y.view(-1).long().cpu()), 0)

    # return total_graph_ids.numpy().flatten(), y_scaler.inverse_transform(total_preds.detach().numpy().flatten().reshape(-1,1)).flatten()

# Change: Change return information 
    return {
        "graph_ids": total_graph_ids.numpy().flatten(),
        "pred_pKd": y_scaler.inverse_transform(total_pKd_preds.numpy()).flatten(),
        "pred_logits": total_bind_logits.numpy().flatten(),
        "pred_prob": torch.sigmoid(total_bind_logits).numpy().flatten()  
    }
...
def make_predictions(config):
    ...
    for i in range(10):
        ...
        # Change: Change to results form 
        results = predict(model, torch.device('cpu'), test_loader, scaler)

        if i == 0:
            df_test = pd.DataFrame({
                'graph_id': results["graph_ids"],
                f'pred_pKd_{i}': results["pred_pKd"],
                f'bind_prob_{i}': results["pred_prob"]
            })
        else:
            df_test[f'pred_pKd_{i}'] = results["pred_pKd"]
            df_test[f'bind_prob_{i}'] = results["pred_prob"]
    ...
    df_test['final_pred_pKd'] = df_test[[f'pred_pKd_{i}' for i in range(10)]].mean(axis=1)
    df_test['final_bind_prob'] = df_test[[f'bind_prob_{i}' for i in range(10)]].mean(axis=1)

    # data = data.merge(df_test, on='graph_id', how='left')
    # Change: Change the merge step
    data = data.merge(df_test[['graph_id', 'final_pred_pKd', 'final_bind_prob']], on='graph_id', how='left')

    """
    Save predictions
    """
    # Change: Change the Saving predictions step 
    print("Saving predictions\n")

#    print("Saving predictions\n")
    os.makedirs("output/predictions", exist_ok=True)
    data.to_csv(f"output/predictions/{config.data_name}_predictions.csv", index=False)
