...
#  Change: Add the methods in order to assess the classification performance 
...
# Change: Change this function in order to include the classification task
def collate_fn(batch):
    """
    Updated for dual-task (regression + classification)
    """
    feature_list, protein_seq_list, reg_label_list, cls_label_list = [], [], [], []
    for _features, _protein_seq, _reg_label, _cls_label in batch:
        feature_list.append(_features)
        protein_seq_list.append(_protein_seq)
        reg_label_list.append(_reg_label)  # regression task
        cls_label_list.append(_cls_label)  # classification task 
    return (
        torch.Tensor(feature_list),
        torch.Tensor(protein_seq_list),
        torch.Tensor(reg_label_list),
        torch.Tensor(cls_label_list)
    )
...
# Change: Add two more functions for classification task
