...
def init_weights(layer):
    ...
    if hasattr(layer, "bias"):
        # Change: Change from True to not None
...
class GraphDataset(InMemoryDataset):
    def __init__(self, root='data', dataset=None,
                 ids=None, y=None, class_y=None, graphs_dict=None, y_scaler=None): # Change: Add the class_y = None
        ...
        # Change: Pass class_y to process
        else:
            self.process(ids, y, class_y, graphs_dict)  
            self.load(self.processed_paths[0])
...
# Change: Change process function, add class_y into the input
    def process(self, ids, y, class_y, graphs_dict):  
        assert len(ids) == len(y) == len(class_y), 'Number of datapoints and labels must be the same'
        data_list = []
        ...
        for i in range(len(ids)):
            ...
            data_point = Data(
                ...
                edge_index=torch.LongTensor(np.array(edge_index)).T,
                edge_attr=torch.Tensor(np.array(edge_features)),
                y=torch.FloatTensor(np.array([y[i]])),          
                class_y=torch.FloatTensor(np.array([class_y[i]]))  
            )
            data_list.append(data_point)
        print('Graph construction done. Saving to file.')
        #self.data, self.slices = self.collate(data_list)
